...
$cus_id = $_POST['cus_id'];
$grp_id = $_POST['grp_id'];
$qry = $pdo->query("SELECT di.`id`,di.`doc_name`,di.`doc_type`,gi.`guarantor_name`,di.`upload`,di.`date_of_noc`, di.`noc_member`, di.`noc_relationship`, di.`noc_status` FROM `document_info` di LEFT JOIN guarantor_info gi ON di.holder_name = gi.id  JOIN  auction_details ad ON di.auction_id = ad.id WHERE di.`cus_id` = '$cus_id'AND 
ad.group_id = '$grp_id'");
if ($qry->rowCount() > 0) {
