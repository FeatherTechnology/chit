...
$c_cr_h_qry = $pdo->query("SELECT SUM(collection_amnt) AS coll_cr_amnt FROM accounts_collect_entry WHERE coll_mode = 1 AND DATE(created_on) <= CURDATE() - INTERVAL 1 DAY "); //Hand Cash
...
$c_cr_b_qry = $pdo->query("SELECT SUM(collection_amnt) AS coll_cr_amnt FROM accounts_collect_entry WHERE coll_mode = 2 AND DATE(created_on) <= CURDATE() - INTERVAL 1 DAY "); //Hand Cash
...
$s_cr_h_qry = $pdo->query("SELECT SUM(settle_cash) AS settlr_cr_amnt FROM settlement_info WHERE settle_type = 1 AND  DATE(created_on) <= CURDATE() - INTERVAL 1 DAY"); //Hand Cash
...
//Expenses Debit.
$e_dr_h_qry = $pdo->query("SELECT SUM(amount) AS exp_dr_amnt FROM expenses WHERE coll_mode = 1 AND DATE(created_on) <= CURDATE() - INTERVAL 1 DAY "); //Hand Cash
...
$e_dr_b_qry = $pdo->query("SELECT SUM(amount) AS exp_dr_amnt FROM expenses WHERE coll_mode = 2 AND DATE(created_on) <= CURDATE() - INTERVAL 1 DAY "); //Hand Cash
...
$ot_cr_h_qry = $pdo->query("SELECT SUM(amount) AS ot_amnt FROM other_transaction WHERE coll_mode = 1 AND type = 1 AND DATE(created_on) <= CURDATE() - INTERVAL 1 DAY "); //Hand Cash //credit
...
$ot_cr_b_qry = $pdo->query("SELECT SUM(amount) AS ot_amnt FROM other_transaction WHERE coll_mode = 2 AND type = 1 AND DATE(created_on) <= CURDATE() - INTERVAL 1 DAY "); //Bank Cash //credit
